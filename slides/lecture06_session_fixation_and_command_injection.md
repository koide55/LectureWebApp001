...
- 回答形式: 提出不要
- 教員が回収する場合のみ: vulnerable モードの特徴を1文で書く
...
1. `Command injection mode` を `vulnerable` にする
2. `/ping` を開く
3. まず `127.0.0.1` を入力して実行する
4. 次に環境に合わせて次のどちらかを入力する
5. `Executed command` と実行結果を比較する
6. 同じ入力を `safe` モードでも試す
...
- 回答形式: 作文
- 文字数目安: 120-200字
- 必ず書くこと: 通常入力の結果、インジェクション入力の結果、`Executed command` の違い、safe モードでの結果

注意:

- この入力は授業用ローカルアプリでのみ試すこと
